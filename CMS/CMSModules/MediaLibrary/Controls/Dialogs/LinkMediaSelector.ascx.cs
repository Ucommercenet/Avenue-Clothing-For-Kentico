    ...
    private void SelectMediaItem(string argument)
    {
        if (!string.IsNullOrEmpty(argument))
        {
            ...
            if (argTable.Count >= 2)
            {
                ...
                {
                    if (LibrarySiteInfo.SiteIsContentOnly)
                    {
                        filePermanentUrl = MediaFileURLProvider.GetMediaFileUrl(fileGuid, fileName);
                        filePermanentUrl = URLHelper.CombinePath(filePermanentUrl, '/', LibrarySiteInfo.SitePresentationURL, null);
                    }
                    else
                    {
