...
");
    ...
    private string GetButtonsScript()
    {
        ...
        switch (SelectionMode)
        {
            ...
            case SelectionModeEnum.SingleButton:
            case SelectionModeEnum.MultipleButton:
                script.AppendLine("function US_Submit(){ SelectItems(escape(ItemsElem().value),HashElem().value); return false; }");
                break;
            ...
            case SelectionModeEnum.MultipleTextBox:
                if (allowEditTextBox)
                {
                    script.AppendFormat("function US_Submit(){{ SelectItems(escape(ItemsElem().value), escape(ItemsElem().value.replace(/^{0}+|{0}+$/g, '')), {1}, {2}, {3}, HashElem().value); return false; }}", ValuesSeparator, hdnClientId, txtClientId, hashElementClientId);
                }
                else
                {
                    script.AppendFormat("function US_Submit(){{ SelectItemsReload(escape(ItemsElem().value), escape(nameElem.value), {0}, {1}, {2}, {3}, HashElem().value); return false; }}", hdnClientId, txtClientId, hdnDrpClientId, hashElementClientId);
                }
                break;
