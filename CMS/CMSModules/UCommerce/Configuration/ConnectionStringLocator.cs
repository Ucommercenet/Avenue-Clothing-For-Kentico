    ...
    public class ConnectionStringLocator : UCommerce.Infrastructure.Configuration.ConnectionStringLocator
    {
        public override string LocateConnectionString()
        {
            ...
            if (string.IsNullOrEmpty(connectionString))
                throw new ConfigurationException("Unable to locate a connection string in connection strings element called 'uCommerce' or 'CMSConnectionString' and connection string configured in CommerceConfiguration does not seem to be valid");
