function ModalController($scope, $rootScope, $compile, $timeout) {
    ...
    $scope.$on('openModalDialog', function (event, args) {
        ...
        dialog.element = $('<div role="dialog" ng-style="modalStyle()" style="height:100%; width:100%;"></div>');
        ...
        var html = $("<iframe src='" + dialog.template + "' ucommerce-stylesheet='" + style + "' class='auto-expand ucommerce-modal-iframe'></iframe>");
...
window.openModal = function (url, name, width, height) {
	angular.element("#ModalControllerContainer").scope().$apply(function (scope) {
		scope.$broadcast('openModalDialog', { template: url, width: width, height: height, scope: scope });
	});
};
